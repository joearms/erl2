...
expr({call,_,{remote,_,{atom,_,shell},{atom,_,Func}},As0}, Bs0, _Lf, _Ef, _RBs) ->
    erl2:local2(shell, Func, As0, Bs0);
expr({call,_,{remote,_,{atom,_,mod},{atom,_,Func}},As0}, Bs0, _Lf, _Ef, _RBs) ->
    erl2:local2(mod, Func, As0, Bs0);
expr({call,_,{remote,_,Mod,Func},As0}, Bs0, Lf, Ef, RBs) ->
    case is_in_script(Mod,Func,length(As0)) of
	true ->
	    {atom,_,AM} = Mod,
	    {atom,_,AF} = Func,
	    erl2:local2(AM, AF, As0, Bs0);
	false ->
...
expr({define2,
      {atom,_,Name},
      {'fun',_Ln,{clauses,Clauses}}
     }, Bs0, _, _, _) ->
    {clause,_Ln,Head,_,_} = hd(Clauses),
    Arity = length(Head),
    D = define_function(Name, Arity, Clauses, Bs0),
    {value, D, Bs0};
...
put1({fundef,{Mod,Func,Sub,Arity}}=K, V) ->
    case get(K) of
	undefined ->
	    io:format("Defun:~p~n",[{Mod,Func,Sub,Arity}]),
	    no_confusing_redefs(Mod,Func,Sub,Arity),
	    put(K, V);
	_ ->
	    io:format("**** Illegal redefinition of function:~p~n",
		      [{Mod,Func,Sub,Arity}]),
	    exit(badScript)
    end;
put1(K, V) ->
    case get(K) of
	undefined ->
	    %% io:format("Putting:~p => ~p~n",[K, V]),
	    %% io:format("Putting:~p~n",[K]),
	    put(K, V);
	_Value ->
	    io:format("**** Illegal put:~p~n",[K]),
	    exit(error)
    end.

no_confusing_redefs(Mod,CF,Func,Arity) ->
    case get({fundef,{theshell,Func,'$$top$$',Arity}}) of
	undefined ->
	    void;
	_ ->
	    io:format("*** You are defining:~p~n",[{Mod,CF,Func,Arity}]),
	    io:format("*** but there is a shell function called ~p/~p~n",
		      [Func,Arity]),
	    io:format("*** please rename one of these functions~n"),
	    exit(badFunctionName)
    end.

save(X) ->
    case get(collectingFunc) of
	undefined ->
	    case get(collectingMod) of
		undefined ->
		    true;
		{Mod, L} ->
		    put1(collectingMod, {Mod, [X|L]})
	    end;
	{Func, L1} ->
...
is_in_script({atom,_,Mod},{atom,_,Func}, Arity) ->
    case get({fundef,{Mod,Func,Arity}}) of
	undefined -> false;
	_ -> true
    end.
